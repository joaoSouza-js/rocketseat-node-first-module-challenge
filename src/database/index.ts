...
class Database {
    ...
    async init(){
        ...
        try {
            const file = await fsPromises.readFile(dbFile,{encoding: "utf-8"})
    ...
    async select<T>(table: tablesAvailable, params:ParamsProps):Promise<T[] | null>{
        const  tableExist = this.checkTableExist(table)
        if(!tableExist) return null

        if(params?.id){
            const content:T[] =  this.database[table].filter(row => row.id === params.id)
            return content
        }
